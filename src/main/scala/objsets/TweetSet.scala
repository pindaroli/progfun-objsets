...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def ascendingByRetweet0(acc:Trending):Trending ={
    remove(elem).ascendingByRetweet0(acc + findMin)
  }
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    if (p(elem))  (accu incl elem) union ((right filter p) union (left filter p)) 
    else accu union ((right filter p) union (left filter p))
  }
...
object GoogleVsApple {
  ...
  def existsGoogle(t:Tweet):Boolean ={
    google.exists(x=>t.text.contains(x))
  }
  
  def existsApple(t:Tweet):Boolean ={
    apple.exists(x=>t.text.contains(x))
  }
  val googleTweets: TweetSet = TweetReader.allTweets.filter(t=>existsGoogle(t));

  val appleTweets: TweetSet = TweetReader.allTweets.filter(t=>existsApple(t));
